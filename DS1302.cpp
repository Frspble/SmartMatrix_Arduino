...
tm DS1302::time() {
  ...
  t.tm_mon = bcdToDec(readIn()) - 1;  // tm_mon is 0-11
  t.tm_wday = bcdToDec(readIn()) - 1;  // tm_wday is 0-6
  t.tm_year = 100 + bcdToDec(readIn());  // tm_year is years since 1900
  return t;
}

void DS1302::time(const tm t) {
  ...
  writeOut(decToBcd(t.tm_mon + 1));  // tm_mon is 0-11
  writeOut(decToBcd(t.tm_wday + 1));  // tm_wday is 0-6
  writeOut(decToBcd(t.tm_year - 100));  // tm_year is years since 1900
